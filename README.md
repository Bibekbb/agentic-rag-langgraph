...
```
                    ...
                    v                                    v
        +-----------------------+            +-----------------------+
        |   Semantic Cache      |            |   LangGraph Agent     |
        |   (Redis, SHA256 key) |            |   (State Machine)     |
        +-----------+-----------+            +-----------+-----------+
                    |                                    |
                HIT |                              MISS  |
                    |                                    v
                    ...
                    |                     |     Multi-query and Rerank   |
                    |                     +------------------------------+
                    |                     |  3. Grade Context            |
                    |                     |     Confidence Scoring       |
                    |                     +------------------------------+
                    |                     |  4. Rewrite (if conf < 0.6)  |
                    |                     |     Loop back to Step 2      |
                    |                     +------------------------------+
                    |                     |  5. Generate Answer          |
...
|  Vector Store  |  |  Cache + Mem   |  | Groq / OpenAI  |
|                |  |                |  |                |
| BGE-M3         |  | Semantic       |  | Routed by      |
| embeddings     |  | cache (1h)     |  | task type      |
| Cosine         |  | Session        |  | Failover       |
| similarity     |  | memory (24h)   |  | support        |
| Metadata       |  | SHA256 keys    |  |                |
| filtering      |  |                |  |                |
+----------------+  +----------------+  +----------------+
```
